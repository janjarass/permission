...
if (!isset($_SESSION['username'])) {
    header('Location: login.php'); // ถ้าไม่ได้เข้าสู่ระบบ จะเปลี่ยนเส้นทางไปหน้าเข้าสู่ระบบ
    exit();
}
...
<section class="section">
    <div class="row">
        <div class="col-lg-12">
            <table class="table datatable" style="width: 100%; border-collapse: collapse; border: 2px solid #ddd;">
                <thead>
                    <tr style="background-color: #f2f2f2;">
                        <th style="text-align: center; font-weight: bold; border: 2px solid #ddd; padding: 8px;">ลำดับ</th>
                        <th style="text-align: center; font-weight: bold; border: 2px solid #ddd; padding: 8px;">รหัสพนักงาน</th>
                        <th style="text-align: center; font-weight: bold; border: 2px solid #ddd; padding: 8px;">สถานะ</th>
                        <th style="text-align: center; font-weight: bold; border: 2px solid #ddd; padding: 8px;">วันที่</th>
                        <th style="text-align: center; font-weight: bold; border: 2px solid #ddd; padding: 8px;">เวลา</th>
                        <th style="text-align: center; font-weight: bold; border: 2px solid #ddd; padding: 8px;">จัดการ</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    $count = 1;
                    $fmt = new IntlDateFormatter('th_TH', IntlDateFormatter::LONG, IntlDateFormatter::NONE, 'Asia/Bangkok', IntlDateFormatter::GREGORIAN, 'd MMMM yyyy'); // ตั้งค่าให้แสดงวันที่เป็นภาษาไทย
                    ...
                    while ($row = $log_result->fetch_assoc()) {
                        ...
                        $time = $date_time->format('H:i:s'); // เวลา

                        echo "<tr>";
                        echo "<td style='text-align: right; border: 2px solid #ddd; padding: 8px;'>" . $count++ . "</td>";
                        echo "<td style='text-align: center; color: blue; border: 2px solid #ddd; padding: 8px;'>" . htmlspecialchars($row['username']) . "</td>";
                        echo "<td style='text-align: center; color: green; border: 2px solid #ddd; padding: 8px;'>" . htmlspecialchars($row['status']) . "</td>";
                        echo "<td style='text-align: center; border: 2px solid #ddd; padding: 8px;'>" . htmlspecialchars($date) . "</td>"; // แสดงวันที่เป็นภาษาไทย
                        echo "<td style='text-align: center; border: 2px solid #ddd; padding: 8px;'>" . htmlspecialchars($time) . "</td>"; // แสดงเวลา
                        echo "<td style='text-align: center; border: 2px solid #ddd; padding: 8px;'>
